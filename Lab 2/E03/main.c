...
enum comando {
    stampa, ordina_d, ordina_c, ordina_p, ordina_a, ricerca, newFile, fine
};

void getN(char *s, int *N);
void leggiFile(char *s, int *N, corsa v[]);
int confronta(corsa c1, corsa c2);
void printCorsa(corsa c);
enum comando leggiComando(char *s);
void ordinamento(corsa *v, enum comando c, int N, int *f, corsa **ord);
...
int main(void){
    ...
    enum comando cmd;
    char com[20];
    corsa *vCorse = (corsa *) malloc(n * sizeof(corsa));
    corsa ***ordM = (corsa ***) malloc(ordina_a * sizeof(corsa **));
    if(vCorse == NULL || ordM == NULL) exit(-1);
    for(int i = 0; i < ordina_a; i++){
        ordM[i] = (corsa **) malloc(n * sizeof(corsa *));
    }

    leggiFile(fileName, &n, vCorse);

    while(continua) {

        printf("Cosa vuoi fare? (stampa, ordina data, ordina codice, ordina partenza, ordina arrivo, ricerca, nuovo file, fine)\n");
        gets(com);
        ...
        switch (cmd) {
            case newFile:
                ...
                vCorse = (corsa *) malloc(n * sizeof(corsa));
                ordM = (corsa ***) malloc(ordina_a * sizeof(corsa **));
                if(vCorse == NULL || ordM == NULL) exit(-1);
                for(int i = 0; i < ordina_a; i++){
                    ordM[i] = (corsa **) malloc(n * sizeof(corsa *));
                }
            ...
            case stampa:
                ...
                break;
            case ordina_d:
                ordinamento(vCorse, ordina_d, n, &flag, ordM[ordina_d - 1]);

                for(int i = 0; i < n; i++){
                    printCorsa(*ordM[ordina_d - 1][i]);
                }

                break;
            case ordina_a:
                ordinamento(vCorse, ordina_a, n, &flag, ordM[ordina_a - 1]);

                for(int i = 0; i < n; i++){
                    printCorsa(*ordM[ordina_a - 1][i]);
                }

                break;
            case ordina_c:
                ordinamento(vCorse, ordina_c, n, &flag, ordM[ordina_c - 1]);

                for(int i = 0; i < n; i++){
                    printCorsa(*ordM[ordina_c - 1][i]);
                }

                break;
            case ordina_p:
                ordinamento(vCorse, ordina_p, n, &flag, ordM[ordina_p - 1]);

                for(int i = 0; i < n; i++){
                    printCorsa(*ordM[ordina_p - 1][i]);
                }
...
enum comando leggiComando(char *s){
    ...
    else if(strcmp("nuovo file", s) == 0) return newFile;
...
void ordinamento(corsa v[], enum comando c, int N, int *flag, corsa **ord){
